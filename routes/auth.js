...
router.post("/login", async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }); // find user based on email
    if (!user) {
    ...
    // Token Generation
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET); // sign id as users id

    // delete password from user object returned
